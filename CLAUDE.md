...
`dari` is the public CLI for Dari (https://dari.dev). It manages routers,
credentials, API keys, and organizations against `https://api.dari.dev`.
End-user docs live at https://docs.dari.dev.
...
go test ./internal/cli/...     # run a single package
```
...
- `internal/cli/` — one file per top-level command group (`auth`, `org`,
  `router`, `apikeys`, `credentials`, `eval`, `activity`, `update`). `root.go`
  composes them.
...
the native install script and GitHub Releases. Don't hand-edit version strings;
the release workflow owns them.
